...
namespace Amsalem.Types.CreditCards
{
    public class AXRertrieveCreditCards : RetrieveCreditCardBase, IRetrieveCardCard
    {
        ...
        {
            var  listManagerBanks = "36";
            string AxDBStatement = ";WITH cte AS(                                                                                    ";
            AxDBStatement += "  SELECT *,Status=1,ROW_NUMBER() OVER (PARTITION BY CREDITCARDNO ORDER BY RECID asc) AS rn                      ";
            AxDBStatement += "  FROM                                                                                                 ";
            ...
            if (!string.IsNullOrEmpty("36"))
            {
                AxDBStatement += "   and AMS_BANKNUMBER  =@listManagerBanks           ";
            }
            AxDBStatement += "   ) d)                                                                                                ";
